...
static zed_token_t zed_instructions[] = { // TODO make the assembler automatically translate jmp, ret, psh, and pop tokens (and also add second function to "cal" token if not calling kfunc)
	{"cad"}, {"mov"},
	{"cnd"}, {"cmp"},
	{"jmp"}, {"cal"},
	{"add"}, {"sub"}, {"mul"}, {"div"},
...
#define ZED_TYPE_INSTRUCTION 255

static inline void rom_add(uint8_t type, uint64_t data) {
	if (type == ZED_TYPE_INSTRUCTION) {
