...
int main(int argc, char* argv[]) {
	printf("[Amber Compiler] Compiling with Amber 1.2.0 (stable) ...\n");

	printf("[Amber Compiler] Parsing arguments ...\n");
	for (int i = 1; i < argc; i++) {
		if (strncmp(argv[i], "--", 2) == 0) { // argument is option
			char* option = argv[i] + 2;

			if (strcmp(option, "help") == 0) {
				printf("Amber Compiler command-line executable help\n");
			...
			fprintf(stderr, "[Amber Compiler] ERROR Unexpected argument `%s`\n", argv[i]);
			return 1;
		}
	}

	printf("[Amber Compiler] Opening output file ...\n");

	yyout = fopen(output_path, "wb");
	if (!yyout) {
		fprintf(stderr, "[Amber Compiler] ERROR Could not open %s for writing\n", output_path);
		return 1;
	}
	...
	printf("[Amber Compiler] Opening input file ...\n");
	chdir(project_path);

	FILE* file = fopen("main.a", "rb");
	if (!file) {
		fprintf(stderr, "[Amber Compiler] ERROR Could not open %s/main.a for reading\n", project_path);
		fclose(yyout);
		return 1;
	}
	...
	printf("[Amber Compiler] Compiling code ...\n");
	yyparse();

	fclose(yyout);
	printf("[Amber Compiler] Done\n");
